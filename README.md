...
## 매일 쓰는 AI 운영 (Daily AI Ops)

레포지토리로 공개하진 않았지만 **실제로 매일 돌리고 있는** AI 운영 워크플로우입니다. 제 AI 활용의 무게중심은 사실 여기에 있습니다 — 화려한 프로젝트가 아니라, 매일의 운영 업무를 AI와 함께 처리하는 방식.

- **Notion MCP × Claude — 공지·안내 멘트 생성**
  기존에 문서화해 둔 Notion DB를 AI가 직접 읽어, 공지·안내 멘트를 매번 처음부터 쓰지 않고 **축적된 문서를 그대로 초안화**합니다. 톤과 맥락을 유지한 채 커뮤니케이션을 빠르게 산출.
- **Intercom MCP × Slack — 유저 문의 반자동 처리**
  현재 오픈된 문의를 AI가 읽어 **매일 Slack에서 "무엇이 남았는지"를 자동 정리**하고, 답변 초안까지 생성합니다. 담당자는 검토·보정 후 직접 전송 — CS·유저 관리를 **Human-in-the-loop 반자동화**로 운영.

> 핵심은 *"AI가 1차 처리, 사람이 판단·확정"*. 놓친 문의와 매번 다시 쓰는 멘트라는 운영 통증을 **매일** 줄입니다.
...
위 *매일 쓰는 AI 운영* 외에, 운영 현장에서 직접 만들어 쓰는 작은 도구들입니다.
...
Not published as repos, but the AI workflows I **actually run every day.** This is where the center of gravity of my AI usage really is — not flashy projects, but handling daily operations *with* AI.

- **Notion MCP × Claude — drafting announcements & user-facing copy**
  The AI reads my already-documented Notion DBs and **drafts straight from accumulated docs** instead of starting from scratch each time — producing comms fast while keeping tone and context.
- **Intercom MCP × Slack — semi-automated ticket handling**
  The AI reads currently open tickets and **auto-summarizes "what's still open" in Slack every day**, drafting replies too. An operator reviews, edits, and sends — running CS / user management as **human-in-the-loop semi-automation.**

> The principle: *"AI does the first pass, the human judges and confirms."* It shaves the daily ops pains of missed tickets and rewritten copy — **every day.**
...
## ⚙️ Other Automation & Data Tools

Beyond the *Daily AI Ops* above, small tools I built and use in operations.
